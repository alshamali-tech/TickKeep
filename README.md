...
- **Zustand + persist** — the ledger lives in `localStorage` (debounced writes, quota-safe compaction, migration-proof merge on load)
...
- **In-app E2E bench** (`#/app/tests`) — 30+ cases including a 1000-client / 1000-project / 1000-invoice / 100-teammate stress run; snapshots and restores your data
...
| Item | Where | Action |
|---|---|---|
| Domain | `index.html`, `public/robots.txt`, `public/sitemap.xml` | Replace `timevault.app` with your real domain (3 files) |
| OG image | `index.html` → `og:image` / `twitter:image` | Host the PNG yourself (e.g. `public/og-image.png`) and point both tags at `/og-image.png`; a local SVG fallback ships today |
| Donation links | `src/app/shell.tsx` → `DONATIONS` | Set your real Ko-fi / Buy Me a Coffee / PayPal handles |
| Legal contact | `src/app/legal.tsx` | Add a contact email; set the governing-law line in Terms |
...
src/
  ...
  app/        pages: dashboard, timer/entries, projects, clients, invoices,
              expenses, estimates, reports (+ builder), year review, team,
